  ...
  for (let i = 0; i < emailConfigs.length; i++) {
    try {
      ...
      if (error.code === 'ETIMEDOUT') {
        console.log(`  → Connection timeout - Port ${emailConfigs[i].config.port} might be blocked by firewall`);
      } else if (error.code === 'ECONNREFUSED') {
        console.log(`  → Connection refused - Server rejected connection`);
      } else if (error.responseCode === 535) {
        console.log(`  → Authentication failed - Check username/password`);
      }
      
      if (i === emailConfigs.length - 1) {
        console.error('⚠️  All email configurations failed');
      }
    }
  }
  
  // If all Gmail configurations fail, try Ethereal (testing service)
  console.log('🧪 All Gmail configurations failed, trying Ethereal test account...');
  try {
    ...
    console.log('✅ Successfully connected to Ethereal test account');
    console.log('📧 Preview emails at: https://ethereal.email');
    console.log(`📧 Test account: ${testAccount.user} / ${testAccount.pass}`);
    return transporter;
  } catch (error) {
    console.error('❌ Ethereal fallback also failed:', error.message);
  }
